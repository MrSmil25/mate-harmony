        ...
        <TabsContent value="profile">
          <section className="academic-card p-5 md:p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Row label="Full name" value={setup?.name ?? "—"} />
              <Row label="University" value={setup?.university ?? "—"} />
              <Row label="Faculty" value={setup?.faculty ?? "—"} />
              <Row label="Study program" value={setup?.program ?? "—"} />
              <Row label="Entry year" value={setup ? `Angkatan ${setup.entryYear}` : "—"} />
              <Row label="Current semester" value={setup ? `Semester ${setup.currentSemester}` : "—"} />
            </div>
          </section>
        </TabsContent>
